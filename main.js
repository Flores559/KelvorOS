...
function scan(p){
 ...
 add('Kelvor Core Loaded',true,'critical','Ascension core online.');
 add('Voice Input Ready',true,'critical','Browser speech recognition enabled where supported.');
...
ipcMain.handle('discord-send-live',async(_e,c={})=>{try{if(!discordReady||!discordClient)throw new Error('Discord is not connected.');if(!c.channelId)throw new Error('Discord channel ID missing.');const channel=await discordClient.channels.fetch(c.channelId);const embed=new EmbedBuilder().setColor(0xff1d1d).setTitle('🔴 THE JD LOUNGE IS LIVE!').setDescription(c.message||'Kelvor has activated the stream.').addFields({name:'Powered By',value:'KelvorOS Ascension',inline:true}).setTimestamp();const sent=await channel.send({embeds:[embed]});logEvent('Discord Announcement Sent',channel.id);return{ok:true,messageId:sent.id,channelId:channel.id};}catch(e){logEvent('Discord Announcement Failed',e.message);return{ok:false,error:e.message};}});
app.whenReady().then(()=>{ensureVault();settingsCache=loadSettings();mainWindow=new BrowserWindow({width:1660,height:1040,minWidth:1280,minHeight:820,backgroundColor:'#030303',title:'KelvorOS v1.1 Beta - Operation Ascension',webPreferences:{preload:path.join(__dirname,'src/core/preload.js'),nodeIntegration:false,contextIsolation:true}});logEvent('KelvorOS Started','Operation Ascension v1.1 Beta online.');mainWindow.loadFile(path.join(__dirname,'src/ui/index.html'));});
