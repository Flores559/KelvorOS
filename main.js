...
function defaultSettings(){return{profileName:'JD',brandName:'The JD Lounge',missionName:'The JD Lounge Live',startingScene:'Starting Soon',gameplayScene:'Gameplay',endingScene:'Stream Ending',liveMessage:'Kelvor has activated the stream. Come hang out with The JD Lounge!',obsHost:'127.0.0.1',obsPort:'4455',obsPassword:'',discordToken:'',discordChannelId:''};}
...
function scan(p){
 ...
 add('Kelvor Core Loaded',true,'critical','Voice Reliability core online.');
 add('Voice Fallback Ready',true,'critical','Typed fallback and command relay enabled.');
...
ipcMain.handle('voice-permission-check',async()=>{try{return{ok:true,platform:process.platform,note:'Microphone permission is requested in the renderer when Push to Talk starts.'};}catch(e){return{ok:false,error:e.message};}});
...
ipcMain.handle('discord-send-live',async(_e,c={})=>{try{if(!discordReady||!discordClient)throw new Error('Discord is not connected.');if(!c.channelId)throw new Error('Discord channel ID missing.');const channel=await discordClient.channels.fetch(c.channelId);const embed=new EmbedBuilder().setColor(0xff1d1d).setTitle('🔴 THE JD LOUNGE IS LIVE!').setDescription(c.message||'Kelvor has activated the stream.').addFields({name:'Powered By',value:'KelvorOS Voice Reliability',inline:true}).setTimestamp();const sent=await channel.send({embeds:[embed]});logEvent('Discord Announcement Sent',channel.id);return{ok:true,messageId:sent.id,channelId:channel.id};}catch(e){logEvent('Discord Announcement Failed',e.message);return{ok:false,error:e.message};}});
app.whenReady().then(()=>{ensureVault();settingsCache=loadSettings();mainWindow=new BrowserWindow({width:1660,height:1040,minWidth:1280,minHeight:820,backgroundColor:'#030303',title:'KelvorOS v1.2 Beta - Voice Reliability',webPreferences:{preload:path.join(__dirname,'src/core/preload.js'),nodeIntegration:false,contextIsolation:true}});logEvent('KelvorOS Started','Voice Reliability v1.2 Beta online.');mainWindow.loadFile(path.join(__dirname,'src/ui/index.html'));});
