...
function scan(p){
 ...
 add('Kelvor Core Loaded',true,'critical','Voice Bridge core online.');
 add('Voice Bridge Ready',true,'critical','Browser recognition, typed fallback, and bridge diagnostics enabled.');
...
function oracleAnswer(text,p){const t=(text||'').toLowerCase();if(t.includes('voice')||t.includes('microphone'))return 'Voice Bridge is active. Use Start Listening, Simulate Voice, or type commands. If browser voice input is unsupported, typed fallback remains fully active.';if(t.includes('obs'))return p.obs.connected?`OBS is connected. Current scene is ${p.obs.currentScene}.`:'OBS is not connected yet. Open Systems and connect OBS.';if(t.includes('discord'))return p.discord.connected?`Discord is online as ${p.discord.botName}.`:'Discord is not connected yet. Open Systems and connect the bot.';if(t.includes('health'))return`System health is ${p.health.score} percent. Status: ${p.health.label}.`;if(t.includes('help')||t.includes('what can you do'))return'I can listen with Push to Talk when supported, run Mission Scan, prepare stream workflows, control OBS, send Discord announcements, open Forge Vault, and report system health.';return'Oracle heard you. Try saying or typing: prepare stream, run mission scan, system health, connect OBS, or send announcement.';}
...
ipcMain.handle('voice-environment-check',async()=>({ok:true,electron:process.versions.electron,chrome:process.versions.chrome,node:process.versions.node,platform:process.platform,note:'Renderer checks browser SpeechRecognition and microphone permission.'}));
...
ipcMain.handle('discord-send-live',async(_e,c={})=>{try{if(!discordReady||!discordClient)throw new Error('Discord is not connected.');if(!c.channelId)throw new Error('Discord channel ID missing.');const channel=await discordClient.channels.fetch(c.channelId);const embed=new EmbedBuilder().setColor(0xff1d1d).setTitle('🔴 THE JD LOUNGE IS LIVE!').setDescription(c.message||'Kelvor has activated the stream.').addFields({name:'Powered By',value:'KelvorOS Voice Bridge',inline:true}).setTimestamp();const sent=await channel.send({embeds:[embed]});logEvent('Discord Announcement Sent',channel.id);return{ok:true,messageId:sent.id,channelId:channel.id};}catch(e){logEvent('Discord Announcement Failed',e.message);return{ok:false,error:e.message};}});
app.whenReady().then(()=>{ensureVault();settingsCache=loadSettings();mainWindow=new BrowserWindow({width:1660,height:1040,minWidth:1280,minHeight:820,backgroundColor:'#030303',title:'KelvorOS v1.3 Beta - Voice Bridge',webPreferences:{preload:path.join(__dirname,'src/core/preload.js'),nodeIntegration:false,contextIsolation:true}});logEvent('KelvorOS Started','Voice Bridge v1.3 Beta online.');mainWindow.loadFile(path.join(__dirname,'src/ui/index.html'));});
