...
let aegisState={shield:'STANDBY',voiceBridge:'READY',nativeVoice:'PLANNED',fallback:'ACTIVE',lastCommand:'None'};

function logEvent(event,detail=''){
  const entry={time:new Date().toISOString(),event,detail};
  timeline.unshift(entry); timeline=timeline.slice(0,350);
  mainWindow?.webContents.send('kelvor-event',{type:'TIMELINE',payload:entry});
  return entry;
}
function defaultSettings(){return{profileName:'JD',brandName:'The JD Lounge',missionName:'The JD Lounge Live',startingScene:'Starting Soon',gameplayScene:'Gameplay',endingScene:'Stream Ending',liveMessage:'Kelvor has activated the stream. Come hang out with The JD Lounge!',obsHost:'127.0.0.1',obsPort:'4455',obsPassword:'',discordToken:'',discordChannelId:'',voiceMode:'hybrid',wakeWord:'kelvor',aegisAutoGuard:true};}
...
function scan(p){
  ...
  add('Aegis Core Loaded',true,'critical','Native voice bridge foundation online.');
  add('Voice Shield Active',true,'critical','Fallback and diagnostics protected.');
...
function oracleAnswer(text,p){
  const t=(text||'').toLowerCase();
  if(t.includes('aegis'))return `Aegis shield is ${aegisState.shield}. Voice fallback is ${aegisState.fallback}. Native voice is ${aegisState.nativeVoice}.`;
  if(t.includes('voice')||t.includes('microphone'))return 'Aegis Voice Core is active. Use Hybrid mode: browser speech if available, simulation if not, and typed fallback always.';
  if(t.includes('obs'))return p.obs.connected?`OBS is connected. Current scene is ${p.obs.currentScene}.`:'OBS is not connected yet. Open Systems and connect OBS.';
  if(t.includes('discord'))return p.discord.connected?`Discord is online as ${p.discord.botName}.`:'Discord is not connected yet. Open Systems and connect the bot.';
  if(t.includes('health'))return`System health is ${p.health.score} percent. Status: ${p.health.label}.`;
  if(t.includes('help')||t.includes('what can you do'))return'I can guard voice input with Aegis, run Mission Scan, prepare stream workflows, control OBS, send Discord announcements, open Forge Vault, and report system health.';
  return'Oracle heard you. Try: Aegis status, prepare stream, run mission scan, system health, connect OBS, or send announcement.';
}
...
ipcMain.handle('oracle-ask',async(_e,text)=>{const p=await payload();const answer=oracleAnswer(text,p);aegisState.lastCommand=text||'None';logEvent('Oracle Response',answer);return{answer,payload:p};});
...
ipcMain.handle('aegis-status',async()=>({ok:true,...aegisState,electron:process.versions.electron,chrome:process.versions.chrome,node:process.versions.node,platform:process.platform}));
ipcMain.handle('aegis-arm',async()=>{aegisState.shield='ARMED';logEvent('Aegis Armed','Voice and command guard active.');return aegisState;});
ipcMain.handle('aegis-standby',async()=>{aegisState.shield='STANDBY';logEvent('Aegis Standby','Voice guard on standby.');return aegisState;});
...
ipcMain.handle('discord-send-live',async(_e,c={})=>{try{if(!discordReady||!discordClient)throw new Error('Discord is not connected.');if(!c.channelId)throw new Error('Discord channel ID missing.');const channel=await discordClient.channels.fetch(c.channelId);const embed=new EmbedBuilder().setColor(0xff1d1d).setTitle('🔴 THE JD LOUNGE IS LIVE!').setDescription(c.message||'Kelvor has activated the stream.').addFields({name:'Powered By',value:'KelvorOS Aegis',inline:true}).setTimestamp();const sent=await channel.send({embeds:[embed]});logEvent('Discord Announcement Sent',channel.id);return{ok:true,messageId:sent.id,channelId:channel.id};}catch(e){logEvent('Discord Announcement Failed',e.message);return{ok:false,error:e.message};}});
app.whenReady().then(()=>{ensureVault();settingsCache=loadSettings();mainWindow=new BrowserWindow({width:1660,height:1040,minWidth:1280,minHeight:820,backgroundColor:'#030303',title:'KelvorOS v1.4 Beta - Operation Aegis',webPreferences:{preload:path.join(__dirname,'src/core/preload.js'),nodeIntegration:false,contextIsolation:true}});logEvent('KelvorOS Started','Operation Aegis v1.4 Beta online.');mainWindow.loadFile(path.join(__dirname,'src/ui/index.html'));});
